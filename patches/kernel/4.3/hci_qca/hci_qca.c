...
static const struct acpi_gpio_params device_wakeup_gpios = { 0, 0, true };
static const struct acpi_gpio_params shutdown_gpios = { 1, 0, true };
...
static int qca_acpi_probe(struct qca_device *dev)
{
	...
	acpi_dev_get_resources(adev, &resources, qca_resource, dev);
        ...
        dev_info(&pdev->dev,"%s, %d", dev->name,dev->init_speed);
        dev_info(&pdev->dev,"%d, %d, %d, %d",desc_to_gpio(dev->device_wakeup),gpiod_get_direction(dev->device_wakeup),gpiod_get_value(dev->device_wakeup),gpiod_is_active_low(dev->device_wakeup));
        dev_info(&pdev->dev,"%d, %d, %d, %d",desc_to_gpio(dev->shutdown),gpiod_get_direction(dev->shutdown),gpiod_get_value(dev->shutdown),gpiod_is_active_low(dev->shutdown));
...
static int qca_probe(struct platform_device *pdev)
{
	...
	gpiod_direction_output(dev->shutdown, true);
	gpiod_direction_output(dev->device_wakeup, true);
